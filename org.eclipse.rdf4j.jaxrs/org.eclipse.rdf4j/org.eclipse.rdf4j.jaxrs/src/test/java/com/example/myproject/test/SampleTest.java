...
public class SampleTest extends KarafTestSupport {
    ...
    public void size() throws IOException {
        ...
        TransactionControllerSize controller = new TransactionControllerSize();
        ...
        try {
            ...
            connectionCheck.setRequestAction(actionSize);

            connectionCheck.connect();
            if (connectionCheck.getResponseCode() != 200) {
                System.out.println("Транзакция не успешна");
            } else {
                //ожидаем что полученный равен 0
                assertEquals(connectionCheck.getResponse().toString, "0");
            }
        ...
        //проверка
        HttpURLConnection connectionCheckAfter = (HttpURLConnection) url.openConnection();
        try {
            connectionCheckAfter.setRequestMethod("PUT");
            connectionCheckAfter.setRequestAction(actionGet);

            connectionCheckAfter.connect();
            if (connectionCheckAfter.getResponseCode() != 200) {
                System.out.println("Транзакция не успешна");
            } else {
                //ожидаем что полученный равен обновленному
                assertEquals(connectionCheckAfter.getResponse().toString, body);
            }
            connectionCheckAfter.disconnect();

            connectionCheckAfter.setRequestMethod("PUT");
            connectionCheckAfter.setRequestAction(actionSize);

            connectionCheckAfter.connect();
            if (connectionCheckAfter.getResponseCode() != 200) {
                System.out.println("Транзакция не успешна");
            } else {
                //ожидаем что полученный не равен 0
                assertNotEquals(connectionCheckAfter.getResponse().toString, 0);
            }
            connectionCheckAfter.disconnect();
        } catch (Exception e) {
            e.printStackTrace();
